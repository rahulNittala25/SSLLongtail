...
TRAIN_LT_PATH = './imagenet_sup_intra_lt_inter_lt_att6_new.json'    # output annotation file: both classes and attributes of training set are long-tailed
TRAIN_KBL_PATH = './imagenet_sup_intra_lt_inter_bl_att6_new.json'   # output annotation file: classes of training set are balanced, but pretext attributes are still iid sampled, i.e., long-tailed

# ============================================================================
# argument parser
parser = argparse.ArgumentParser()
parser.add_argument('--data_path', default='/data4/imagenet/ILSVRC/Data/CLS-LOC/', type=str, help='indicate the path of CLS-LOC folder for ImageNet.')
...
class ImageNetExtract(Dataset):
    def __init__(self, index, data_path):
        ...
        categories = list_directories(data_path)
        names = list_directories(os.path.join(data_path, categories[index]))
...
# ============================================================================
# generate GLT splits
def generate_train_val_test(cls_data, VAL_SIZE=5, TEST_SIZE=10, NUM_CAT=1000, NUM_ATT=3, intra_type='lt', inter_type='bl'):
    # trainset =         {'label':{}, 'frequency':{}, 'attribute':{}}
    train_l_set =      {'label':{}, 'frequency':{}, 'attribute':{}}	
    train_ul_set =     {'label':{}, 'frequency':{}, 'attribute':{}}
    ...
    # intra distribution
    if(NUM_ATT==6):
        intra_dist = [40,30,13,7,5,5]
    else:
        intra_dist = [int(100*((1/10)**(i/(NUM_ATT-1)))) for i in range(NUM_ATT)]
    ...
    if intra_type == 'lt' and inter_type == 'lt':
        ...
        cls_data = get_long_tailed_data(cls_data, intra_dist)
    elif intra_type == 'lt' and inter_type == 'bl':
    ...
    cls_data, train_l_set = generate_labelled_set(cls_data, train_l_set, fraction=0.1, matching = True)

    for key, val in cls_data.items():
        for c_key, c_val in val.items():
            for item in c_val:
                #Previously trainset
                train_ul_set['label'][item] = key
                train_ul_set['frequency'][item] = get_frequency_label(key)
                train_ul_set['attribute'][item] = c_key
    
    print('====== Total Num of unlabelled Train Set is {} ========'.format(len(train_ul_set['label'])))
    
    return train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl
...
def generate_intra_lt_test(cls_data, input_set, intra_dist, VAL_SIZE, NUM_ATT):
    ...
    multi_weight = TOTAL_SIZE / sum(intra_dist)
    
    for key, val in cls_data.items():
        for i, (c_key, c_val) in enumerate(val.items()):
            att_size = round(intra_dist[i] * multi_weight)
            for _ in range(att_size):
                item = c_val.pop(0)
                input_set['label'][item] = key
                input_set['frequency'][item] = get_frequency_label(key)
...
def generate_iid_set(cls_data, input_set, VAL_SIZE, NUM_ATT):
    ...
    new_cls_data = {}
    for key, val in cls_data.items():
        new_cls_data[key] = {str(i):[] for i in range(NUM_ATT)}
...
def generate_labelled_set(cls_data, input_set, fraction=0.1, NUM_ATT = 6, matching = True):	
    import math	
    for key, val in cls_data.items():	
        for c_key, c_val in val.items():	
            tmp = len(c_val)	
#            labelled_count = max(1,round(tmp*fraction))
            labelled_count = round(tmp*fraction)
            if(tmp==0):
                print(str(key)+" "+str(c_key))	
            for i in range(labelled_count):	
                item = c_val.pop(0)	
                input_set['label'][item] = key	
                input_set['frequency'][item] = get_frequency_label(key)	
                input_set['attribute'][item] = c_key	
    print('====== Total Num of Labelled Train Set is {} ========'.format(len(input_set['label'])))	
    return cls_data, input_set
...
def save_output_ssl(train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, idx2label, output_path):	
    output_dict = {}	
    ...
    output_dict['train_l'] = train_l_set	
    output_dict['train_ul'] = train_ul_set	
...
def main(args):
    ...
    out_paths_dict = {}
    category_dict = {}
    cls_path = "./cls_data_att6_new.json"
    idx2label_path = "./idx_to_label_att6_new.json"

    if(not (os.path.exists(cls_path) or os.path.exists(idx2label_path))):
        for index in range(len(list_directories(args.data_path))):
        ...
        print("Creating cls_data and idx_to_label jsons....")
        with open(cls_path, 'w') as fp:
            json.dump(cls_data, fp)
        with open(idx2label_path, 'w') as fp:
            json.dump(idx2label, fp)
        print("Files successfully created!")
    
    else:
        print("cls_data and idx_to_label files found. Loading them....")
        with open(cls_path, 'r') as fp:
            cls_data = json.load(fp)
        with open(idx2label_path, 'r') as fp:
            idx2label = json.load(fp)
        print("Loading successful!")

    train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl = generate_train_val_test(cls_data, intra_type='lt', inter_type='lt', NUM_ATT = 6)
    save_output_ssl(train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, idx2label, TRAIN_LT_PATH)


    # Train-KLT and corresponding (Test-LT + Test-KBL + Test-GBL)
    # cls_data, idx2label = cluster_ranking(clusterlabel_dict)
    # trainset, valset, testset_lt, testset_bl, testset_bbl = generate_train_val_test(cls_data, intra_type='lt', inter_type='bl', NUM_ATT = 6)
