...
def generate_train_val_test(cocottributes, key2cats, TEST_SIZE=100, VAL_SIZE=50, intra_type='lt', inter_type='lt'):
    ...
    if intra_type == 'lt' and inter_type == 'lt':
        train_ul_set = generate_data(cat_keys, ann_vecs, train_ul_set, CAT2FRQ, CAT2ID, imb_type='exp')
    elif intra_type == 'lt' and inter_type == 'bl':
        train_ul_set = generate_data(cat_keys, ann_vecs, train_ul_set, CAT2FRQ, CAT2ID, imb_type='bl')
    else:
        raise ValueError('Wrong Combination of Distribution Types')
    print('Data Size After {} is {}'.format('Selection', len(train_ul_set['label'])))
    ...
    #################################################
    if intra_type == 'lt' and inter_type == 'lt':
        testset_lt = generate_iid_set(train_ul_set, testset_lt, TEST_SIZE * NUM_CAT)
    print('Data Size After {} is {}'.format('Selection', len(train_ul_set['label'])))
    ...
    #################################################
    valset = generate_iid_set(train_ul_set, valset, VAL_SIZE * NUM_CAT)
    print('Data Size After {} is {}'.format('Selection', len(train_ul_set['label'])))

    train_l_set = generate_labeled_set(train_ul_set, train_l_set, 0.1)
    print('Data Size After {} is {}'.format('Selection', len(train_ul_set['label'])))
    
    return train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, CAT2ID
...
def generate_labeled_set(inputset, outputset, fraction=0.1):
    keys = {}
    cls_size = len(set(inputset['label'].values()))
    for i in range(cls_size):
        keys[i] = []
    for key,value in inputset['label'].items():
        keys[value].append(key)
    for i in range(cls_size):
        new_keys = random.sample(keys[i], int(len(keys[i])*fraction))
        for key in new_keys:
            outputset['label'][key] = inputset['label'][key]
            outputset['frequency'][key] = inputset['frequency'][key]
            outputset['attribute'][key] = inputset['attribute'][key]
...
def save_output(cocottributes, train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, CAT2ID, coco_data, output_path):
    output_dict = {}
    output_dict.update(cocottributes)
    
    print('Labeled Train size is : {}'.format(len(train_l_set['label'])))
    print('Unlabeled Train size is : {}'.format(len(train_ul_set['label'])))
...
# generate splits
train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, CAT2ID = generate_train_val_test(cocottributes, key2cats, TEST_SIZE=TEST_SIZE, VAL_SIZE=VAL_SIZE, intra_type='lt', inter_type='bl')
save_output(cocottributes, train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, CAT2ID, coco_data, TRAIN_KBL_PATH)
...
att_statistics, cat_statistics, key2cats = get_statistics(cocottributes, cocottributes['ann_vecs'], coco_data)
# generate splits
train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, CAT2ID = generate_train_val_test(cocottributes, key2cats, TEST_SIZE=TEST_SIZE, VAL_SIZE=VAL_SIZE, intra_type='lt', inter_type='lt')
save_output(cocottributes, train_l_set, train_ul_set, valset, testset_lt, testset_bl, testset_bbl, CAT2ID, coco_data, TRAIN_LT_PATH)
