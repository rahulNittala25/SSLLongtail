...
from Data.ms_coco_glt import get_mscoco_glt
...
parser.add_argument('--img_size', type=int, default=112, help='MSCOCO_64 or MSCOCO_112')
...
parser.add_argument('--lam_unsup', default=1.0, type=float, help='relative weight of unsupervised loss')
# Miscs
parser.add_argument('--manualSeed', type=int, default=0, help='manual seed')
parser.add_argument('--gpu', default='0', type=str, help='id(s) for CUDA_VISIBLE_DEVICES')
parser.add_argument('--annotation-path', default = "/BS/SSLLongTail/nobackup/COCO_BL_balanced/coco_cbl/annotation.json", type=str, help="The path to the annotations json file with data splits")
...
def main():
    ...
    # Data
    print(f'==> Preparing MS_COCO_GLT-{args.img_size}')

    
    tmp = get_mscoco_glt(args.annotation_path, args.phase, img_size=args.img_size, return_strong_labeled_set=False)
    ...
    def create_model(ema=False):
        ...
        print("Available devices are:"+str(available_devices))
        model = nn.DataParallel(models.ResNet50(num_classes=num_class, rotation=True, classifier_bias=True), available_devices)
        model.to(device)
        if ema:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        *train_info, = train(labeled_trainloader,
        ...
        print("Loss: "+str(test_loss)+" Accuracy:"+str(test_acc))
    ...
    print('Mean bAcc:')
    print(np.mean(test_accs[-20:].cpu()))

    print('Mean GM:')
    print(np.mean(test_gms[-20:].cpu()))
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        try:
            inputs_x, targets_x, _, _ = next(labeled_train_iter)
        except:
            labeled_train_iter = iter(labeled_trainloader)
            inputs_x, targets_x, _, _ = next(labeled_train_iter)

        try:
            (inputs_u, inputs_u2), gt_targets_u, _, _ = next(unlabeled_train_iter)
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            (inputs_u, inputs_u2), gt_targets_u, _, _ = next(unlabeled_train_iter)
        ...
        with torch.no_grad():
            ...
            total_acc = p_hat.cpu().eq(gt_targets_u).float().view(-1).cuda()
        ...
        print("Batch time: "+str(time.time() - end))
        end = time.time()
...
def validate(valloader, model, criterion, use_cuda, mode):
    ...
    classwise_correct = torch.zeros(num_class).to(device)
    classwise_num = torch.zeros(num_class).to(device)
